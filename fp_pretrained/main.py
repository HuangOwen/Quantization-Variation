...
from engine import initialize_quantization, train_one_epoch, evaluate, train_one_epoch_tb, initialize_muitihead_quantization, head_analysis
from losses import DistillationLoss
from samplers import RASampler
from models import swin, vit, SReT
...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data-path', default='/home/zhiqiang.shen/users/jingyechen/dataset/imagenet/', type=str,
                        help='dataset path')
    ...
    parser.add_argument('--stage-ratio', type=float, default=0., help="searching stage epochs / total epochs. For uniform quant should be 0")
    parser.add_argument('--budget', type=float, default=0., help='bitops budget(G)')
    parser.add_argument('--save-ptq', action='store_true', default=False)

    parser.add_argument('--reg', action='store_true', default=False)
    parser.add_argument('--track-osc', action='store_true', default=False)
    return parser
...
def main(args):
    ...
    if args.resume == '' and (args.abits > 0 or args.wbits > 0) and utils.is_main_process():
        print("Starting quantization scale initialization")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_tb(
            model, criterion, data_loader_train,
            optimizer, device, epoch, loss_scaler,
            args.clip_grad, model_ema, mixup_fn, args.reg, args.track_osc,
            set_training_mode=args.finetune == '', writer=writer, total_epochs = args.epochs, bitops_scaler = args.bitops_scaler, budget = args.budget, output_dir = output_dir
        )
